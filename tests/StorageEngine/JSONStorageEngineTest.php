...
use Backup\StorageEngine\JSONStorageEngine;
use Backup\StorageEngine\SQLiteStorageEngine;
use Backup\User\AmazonS3User;
use PHPUnit\Framework\TestCase;

class JSONStorageEngineTest extends TestCase
{
    /** @var JSONStorageEngine */
...
In order to prevent unwanted desturction, delete this file before running the tests.",
    ...
    {
        $tempDB = tempnam(__DIR__, 'TempJSONTest');
        $storageEngine = new JSONStorageEngine($tempDB);
     ...
     * @expectedExceptionMessage User user already exists in the JSON. Cannot add this user.
     */
    ...
    public function testNonExistentFileWorks()
    {
        $name = tempnam(__DIR__, 'TempJSONTest');
        unlink($name);

        new JSONStorageEngine($name);
    }
}